...
## Where am I? ####
here::i_am("1_src/0_prep_data1.R")
...
vars <- grep("^pca[1-8]$|^sdo[1-8]$|^aut[1-4]$|^adem[1-3]$|^wave$|^ID$", names(raw), value = T)
raw[,wave:=gsub("e5","",wave)]
raw <- raw[wave %in% paste0("w",1:5),..vars]

## Reverse code ####
vars <- c("sdo3","sdo4","sdo7","sdo8","adem2","adem3")
...
# SAVE ####
## Alpha ####
xlsx::write.xlsx(alpha_total, "2_output/1_alpha/alpha_v1.xlsx", 
                 sheetName="total", 
                 row.names = FALSE,
                 append=F)

for (i in names(alpha_drop)) {
  xlsx::write.xlsx(alpha_drop[[i]], "2_output/1_alpha/alpha_v1.xlsx", 
...
## Data ####
write.csv(dt,file=here::here("0_data","data_sel_v1.csv"),row.names = F)
